...
/*
    TODO
        falta adaptar a las nuevas entradas de la BD
*/
...
class UsuariosBD {
    ...
    function setUsuario($nombre, $apellidos, $DNI, $telefono, $calle, $numero, $piso, $puerta, $mail, $username, $passwd) {
        $correcto = false;
        $pdo = new AccesoBD();
        $pdo = $pdo->abrirConexion();

        $insertar = $pdo->prepare("INSERT into Usuarios (id, nombre, apellidos, DNI, telefono, calle, numero, piso, puerta, mail, id_rol) VALUES (0,?,?,?,?,?,?,?,?,?,1)");
        if ($insertar->execute(array($nombre, $apellidos, $DNI, $telefono, $calle, $numero, $piso, $puerta, $mail))) {
    ...
    function getUsuarioByUsername($username){
        $pdo = new AccesoBD();
        $pdo = $pdo->abrirConexion();
        $result = $pdo->query("SELECT * FROM usuarios WHERE id = (SELECT id_usuario FROM sesiones WHERE username = '$username')");
        $usuario = null;
        if($result->rowCount() > 0){
